...
function startdriver {
    ...
    if [ -e $builddir/drivers/$thedriver ]; then
        ...
        if [ -e $basedir/drivers/$thedriver ]; then
            ...
            echo "Using Python adaptor $thedriver"
        else
    ...
    PYTHONPATH=/home/pi/cbr-proto2/build/lib $drvfile $driverparams &
    pids+=($!)
    # Print last element
    echo "Driver ${pids[@]: -1}"
    #echo "drvfile=$drvfile, driverparams=$driverparams"
}
...
function laptopsensortag {
    ...
    driverparams="hci1 BC:6A:29:AB:42:8E $theapp:insensortag"
    removesockets
    startaggregator
    startaggregatorchannel
    startdriver
...
function rpisensortag {
    ...
    theapp=sensortagapp
    thedriver=sensortagdriver.py
    driverparams="hci0 BC:6A:29:AB:42:8E $theapp:insensortag"
    removesockets
    echo "manager starting aggregator"
    startaggregator
    startaggregatorchannel
    echo "manager starting driver"
    startdriver
